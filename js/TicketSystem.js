...
function ticketSystemParseWindowResults(myWindow, callback) {
  myWindow.request_start_date = getDateInCentralTime(myWindow.request_start_date); // eslint-disable-line camelcase
  myWindow.request_stop_date = getDateInCentralTime(myWindow.request_stop_date); // eslint-disable-line camelcase
  myWindow.request_stop_date.setHours(23);
  myWindow.request_stop_date.setMinutes(59);
  myWindow.mail_start_date = getDateInCentralTime(myWindow.mail_start_date); // eslint-disable-line camelcase
  myWindow.current = getDateInCentralTime(myWindow.current);
  myWindow.year = myWindow.year*1;
  if(myWindow.test_mode === '') { // eslint-disable-line camelcase
    myWindow.test_mode = false; // eslint-disable-line camelcase
  }
    ...
    if(this.objectType !== undefined) {
      if(Array.isArray(jqXHR.responseJSON)) {
        for(var i = 0; i < jqXHR.responseJSON.length; i++) {
          jqXHR.responseJSON[i] = new this.objectType(jqXHR.responseJSON[i], this.ticketSystem); // eslint-disable-line security/detect-object-injection
        }
      }
    }
